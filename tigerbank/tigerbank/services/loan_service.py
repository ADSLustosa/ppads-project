...
def hire_loan(account_id: int, principal: float, months: int, monthly_rate: float=0.025) -> Loan:
    ...
    with db.session.begin():
        ...
        loan = Loan(account_id=acc.id, principal=p, months=months, monthly_rate=monthly_rate, installment_amount=installment, remaining_installments=months)
        db.session.add(loan)
        acc.balance = _as_decimal(acc.balance) + p
        tx = Transaction(account_id=acc.id, kind="Empréstimo Aprovado", amount=p, description=f"Empréstimo - {months}x de {installment.quantize(Decimal('0.01'))}", balance_after=acc.balance)
        db.session.add(tx)
        return loan
