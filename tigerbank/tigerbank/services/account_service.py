...
def deposit(account_id: int, amount: float, description: str="Depósito") -> Transaction:
    ...
    with db.session.begin():
        ...
        tx = Transaction(account_id=acc.id, kind="Depósito", amount=amt, description=description, balance_after=new_balance)
        db.session.add(tx)
        return tx
...
def withdraw(account_id: int, amount: float, description: str="Saque") -> Transaction:
    ...
    with db.session.begin():
        ...
        new_balance = _as_decimal(acc.balance) - amt
        acc.balance = new_balance
        tx = Transaction(account_id=acc.id, kind="Saque", amount=-amt, description=description, balance_after=new_balance)
        db.session.add(tx)
        return tx
