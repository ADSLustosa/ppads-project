...
def transfer(from_account_id: int, to_account_id: int, amount: float,
    ...
    with db.session.begin():
        ...
        db.session.add(Transaction(
            account_id=orig.id, kind="Transferência Enviada", amount=-amt,
            description=f"{description} - Para: {dest.user.name}", balance_after=orig.balance))
        db.session.add(Transaction(
            account_id=dest.id, kind="Transferência Recebida", amount=amt,
            description=f"{description} - De: {orig.user.name}", balance_after=dest.balance))
